...
class PdfTemplate extends \Controller
{
	...
	public function __construct($strFormat = 'A4', $strOrientation = PdfTemplate::ORIENTATION_PORTRAIT,
		$intMarginLeft = 15, $intMarginRight = 15, $intMarginTop = 16, $intMarginBottom = 16,
		$intMarginHeader = 9, $intMarginFooter = 9)
	{
		$this->objPdf = new \mPDF('', $strFormat, 0, '', $intMarginLeft, $intMarginRight, $intMarginTop,
			$intMarginBottom, $intMarginHeader, $intMarginFooter, $strOrientation);
	}
	...
	{
		$this->objPdf->SetImportUse();
		$intPageCount = $this->objPdf->SetSourceFile(TL_ROOT . '/' . ltrim($strFilename, '/'));
		$this->objPdf->UseTemplate($this->objPdf->ImportPage($intPageCount));
	}
	...
	public function writeHtml($strHtml, $strCss = '')
	{
		...
		$this->objPdf->WriteHTML($this->replaceInsertTags($strHtml));
	}
